...
import aaaShadowDiffusion.model.networks as networks
...
class DDPM(BaseModel):
    ...
    def optimize_parameters(self):
        self.optG.zero_grad()
        l_pix = self.netG(self.data)  # read the json and run on the json model
    ...
    def test(self, input, continous=False):
        self.netG.eval()
        with torch.no_grad():
            self.new_mask = torch.zeros_like(input)
            self.SR = self.netG.super_resolution(input, self.new_mask, continous)
        return self.SR
        # self.netG.train()
