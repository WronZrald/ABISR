...
class GaussianDiffusion(nn.Module):
    def __init__(
        self,
        denoise_fn,  # is the model take in
        image_size,
        channels=3,
        loss_type='l1',
        conditional=True,
    ...
    def p_losses(self, x_in, noise=None):
        ...
        e = torch.randn_like(x_start)  # e is noise
        x_noisy = x_start * a.sqrt() + e * (1.0 - a).sqrt()
        # x_recon, updated_mask = self.denoise_fn(
            # torch.cat([x_in['SR'], x_in['mask'], x_noisy], dim=1), t.float())
        x_recon = self.denoise_fn(  # bring the args to the model(Unet)
            torch.cat([x_in['SR'], x_noisy], dim=1), t.float())  # now 6 channels
