...
class ImageDataset(Dataset):
    @torch.no_grad()
    def __init__(self, train_lr_paths, degrade_hr_paths, train_hr_paths, diffusion_sr_paths):
    # def __init__(self, train_lr_paths, degrade_hr_paths, train_hr_paths):
        ...
        self.files_diffusion = diffusion_sr_paths
    ...
    def __getitem__(self, index):
        ...
        img_diffusion = cv2.imread(self.files_diffusion[index % len(self.files_diffusion)])

        # Augmentation
        if random.random() < opt["augment_prob"]:
            img_lr, img_degrade_hr, img_hr, img_diffusion = self.augment([img_lr, img_degrade_hr, img_hr, img_diffusion])
        ...
        img_diffusion = self.transform(img_diffusion)


        return {"lr": img_lr, "degrade_hr": img_degrade_hr, "hr": img_hr, "diff": img_diffusion}
