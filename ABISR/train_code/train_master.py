...
class train_master(object):
    ...
    def master_run(self):
        ...
        diffusion_sr_paths = glob.glob(self.options["diffusion_sr_path"] + "/*.*")
        train_dataloader = DataLoader(ImageDataset(train_lr_paths, degrade_hr_paths, train_hr_paths, diffusion_sr_paths), batch_size=self.batch_size, shuffle=True, num_workers=self.n_cpu)  # ONLY LOAD HALF OF CPU AVAILABLE
        ...
        for epoch in range(start_epoch, n_epochs):
            ...
            for batch_idx, imgs in enumerate(tqdm_bar):
                ...
                imgs_diffusion = imgs["diff"].cuda()

                # tmp = imgs_lr[:, [2, 1, 0], :, :]  # trans to rgb
                # imgs_diffusion = diffusion.test(tmp, continous=True)
                # imgs_diffusion = imgs_diffusion[:, [2, 1, 0], :, :].cuda()  # trans to bgr

                # Used for each iteration
                self.generator_loss = 0
                self.single_iteration(imgs_lr, imgs_degrade_hr, imgs_hr, imgs_diffusion)
    ...
    def single_iteration(self, imgs_lr, imgs_degrade_hr, imgs_hr, imgs_diffusion):
        ...
        out = F.interpolate(imgs_diffusion, size=(64, 64), mode='bilinear')
        concatenated = torch.cat((imgs_lr, out), dim=1)

        # concatenated = torch.cat((imgs_lr, imgs_diffusion), dim=1)

        with torch.cuda.amp.autocast():
            # generate high res image
            gen_hr = self.generator(concatenated)
            # gen_hr = self.generator(imgs_lr, out)
